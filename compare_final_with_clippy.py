...
def evaluate_clippy():
    with open("clippy_results.json") as f:
        data = json.load(f)
    tp = fp = tn = fn = 0
    for path, res in data.items():
        actual = "vuln_" in path
        # 'has_finding' might be missing if there was an error, default to False
        pred = res.get("has_finding", False)
        if actual and pred: tp += 1
        elif actual and not pred: fn += 1
        elif not actual and pred: fp += 1
        else: tn += 1
...
print_metrics("Clippy (Linter)", evaluate_clippy())